...
const esc = (v: unknown) => String(v ?? "").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\uFFFE\uFFFF]/g, "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
function col(n: number) { let s = ""; while (n > 0) { n -= 1; s = String.fromCharCode(65 + n % 26) + s; n = Math.floor(n / 26); } return s; }
...
function cat(parts:Uint8Array[]){const z=parts.reduce((a,p)=>a+p.length,0),o=new Uint8Array(z);let x=0;for(const p of parts){o.set(p,x);x+=p.length;}return o;}
function zip(files:Array<{name:string;data:string|Uint8Array}>){const l:Uint8Array[]=[],c:Uint8Array[]=[];let off=0;for(const f of files){const n=encoder.encode(f.name),d=typeof f.data==="string"?encoder.encode(f.data):f.data,r=crc32(d);const a=cat([u32(0x04034b50),u16(20),u16(0),u16(0),u16(0),u16(0),u32(r),u32(d.length),u32(d.length),u16(n.length),u16(0),n,d]);l.push(a);c.push(cat([u32(0x02014b50),u16(20),u16(20),u16(0),u16(0),u16(0),u16(0),u32(r),u32(d.length),u32(d.length),u16(n.length),u16(0),u16(0),u16(0),u16(0),u32(0),u32(off),n]));off+=a.length;}const cd=cat(c);return cat([...l,cd,cat([u32(0x06054b50),u16(0),u16(0),u16(files.length),u16(files.length),u32(cd.length),u32(off),u16(0)])]);}

const S={normal:0,title:1,header:2,money:3,subtotal:4,section:5,number:6,result:7,bold:8};
const tc=(r:string,v:unknown,s=S.normal)=>`<c r="${r}" s="${s}" t="inlineStr"><is><t>${esc(v)}</t></is></c>`;
const nc=(r:string,v:number,s=S.number)=>`<c r="${r}" s="${s}" t="n"><v>${Number.isFinite(v)?v:0}</v></c>`;
const fc=(r:string,f:string,v:number,s=S.money)=>`<c r="${r}" s="${s}"><f>${esc(f.replace(/^=/,""))}</f><v>${Number.isFinite(v)?v:0}</v></c>`;
const row=(r:number,cells:string[],h=18)=>`<row r="${r}" ht="${h}" customHeight="1">${cells.join("")}</row>`;
function sheet(dim:string,rows:string[],cols:string,merge:string,freeze=3){return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="${dim}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="${freeze}" topLeftCell="A${freeze+1}" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols>${cols}</cols><sheetData>${rows.join("")}</sheetData>${merge?`<mergeCells count="1"><mergeCell ref="${merge}"/></mergeCells>`:""}</worksheet>`;}
const yf=(result:BudgetResult,year:number):AnnualFlow=>result.annualFlows.find(f=>f.year===year)??result.annualFlows[0];
function cohorts(budget:CohortBudget,flow:AnnualFlow,year:number){const a=budget.annualOverrides.find(x=>x.year===year);const c=[a?.directionProrated&&a.directionAllocationRate>0?1/a.directionAllocationRate:0,a?.assistanceProrated&&a.assistanceAllocationRate>0?1/a.assistanceAllocationRate:0,flow.direction>0&&a?.annualDirection?a.annualDirection/flow.direction:0].filter(x=>Number.isFinite(x)&&x>=1);return Math.max(1,Math.round(c[0]??1));}
function enrollmentStudents(flow:AnnualFlow,fee:number){return fee>0?flow.grossEnrollmentFee/fee:0;}
function maintenanceStudents(flow:AnnualFlow,monthly:number){return monthly>0?flow.maintenanceScholarships/(monthly*12):0;}

function parametersSheet(budget:CohortBudget,result:BudgetResult,parameters:InstitutionalParameters){const ys=result.years,last=col(ys.length+1),supportLabel=budget.program.type==="DOCTORADO"?"Congresos y pasantías de doctorado":"Congresos y pasantías",rows:string[]=[row(1,[tc("A1",`Parámetros ${ys[0]}–${ys.at(-1)} - ${budget.program.name}`,S.title)],28),row(2,[]),row(3,[tc("A3","Parámetro",S.header),...ys.map((y,i)=>nc(`${col(i+2)}3`,y,S.header))],22)];const labels=["Arancel anual","Matrícula","Valor hora docencia","Académico/a Claustro jornada parcial","Revisión de tesis","Beca de atención económica (manutención)","Meses de manutención","Año de inicio","Director/a Programa","Asistente de Dirección (media jornada)","Cohortes/versiones activas para prorrateo",supportLabel];labels.forEach((label,k)=>{const r=4+k,cells=[tc(`A${r}`,label)];ys.forEach((y,i)=>{const f=yf(result,y),a=resolvedAnnualOverrideForYear(budget,parameters,y),co=cohorts(budget,f,y);let v=0;if(r===4)v=a.annualTuition;else if(r===5)v=a.annualEnrollmentFee;else if(r===6)v=a.directTeachingHourValue;else if(r===8)v=a.thesisGuidancePerGraduatingStudent;else if(r===9)v=a.maintenanceScholarshipMonthlyValue;else if(r===10)v=f.maintenanceScholarships>0?12:0;else if(r===11)v=i===0?budget.startYear:0;else if(r===12)v=f.direction*co;else if(r===13)v=f.assistance*co;else if(r===14)v=co;else if(r===15)v=f.congressesInternships*co;cells.push(nc(`${col(i+2)}${r}`,v,(r===10||r===11||r===14)?S.number:S.money));});rows.push(row(r,cells));});return sheet(`A1:${last}15`,rows,`<col min="1" max="1" width="46" customWidth="1"/><col min="2" max="${ys.length+1}" width="17" customWidth="1"/>`,`A1:${last}1`);}
...
function totalSheet(budget:CohortBudget,result:BudgetResult){const ys=result.years,last=col(ys.length+1),supportLabel=budget.program.type==="DOCTORADO"?"Congresos y Pasantías de doctorado (al tercer año de la cohorte)":"Congresos y pasantías",rows:string[]=[row(1,[tc("A1",`${budget.program.name} - Cohorte ${budget.startYear}`,S.title)],28),row(2,[]),row(3,[tc("A3","DETALLE",S.header),...ys.map((y,i)=>nc(`${col(i+2)}3`,y,S.header))],22)];const vals=ys.map(y=>{const f=yf(result,y);return {f,academic:[f.directTeachingCost,f.replacementTeachingCost,f.thesisGuidanceCost],nonAcademic:[f.direction,f.assistance,f.otherNonAcademicHonoraria],books:f.booksPublications,diff:f.diffusion,travel:f.travelFreight+f.perDiem,software:f.software,oper:f.operational+f.otherCosts,food:f.foodBeverages,congress:f.congressesInternships,aid:f.scholarshipsAndAid,equip:f.equipment};});const direct=(r:number,label:string,get:(v:typeof vals[number])=>number,style=S.money)=>rows.push(row(r,[tc(`A${r}`,label),...vals.map((v,i)=>nc(`${col(i+2)}${r}`,get(v),style))]));rows.push(row(4,[tc("A4","INGRESOS",S.section),...ys.map((_,i)=>tc(`${col(i+2)}4`,"",S.section))]));direct(5,"Ingresos por Matrícula",v=>v.f.grossEnrollmentFee);direct(6,"Ingresos por Aranceles",v=>v.f.tuitionAfterBenefits);direct(7,"Otros ingresos",v=>v.f.externalIncome+v.f.institutionalFinancing+v.f.otherIncome);rows.push(row(8,[tc("A8","INGRESOS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}8`,`SUM(${col(i+2)}5:${col(i+2)}7)`,v.f.grossEnrollmentFee+v.f.tuitionAfterBenefits+v.f.externalIncome+v.f.institutionalFinancing+v.f.otherIncome,S.subtotal))]));rows.push(row(9,[]));direct(10,"Docentes / Docentes (compromiso académico)",v=>v.academic[0]);direct(11,"Docentes Claustro Jornada Parcial 16h semanales (*)",()=>0);direct(12,"Docentes Externos a honorario",v=>v.academic[1]);direct(13,"Revisión de tesis externos a honorarios",v=>v.academic[2]);rows.push(row(14,[tc("A14","COSTOS ACADÉMICOS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}14`,`SUM(${col(i+2)}10:${col(i+2)}13)`,v.f.academicHonoraria,S.subtotal))]));rows.push(row(15,[]));direct(16,"Honorarios Director programa",v=>v.nonAcademic[0]);direct(17,"Honorarios Asistante de Dirección",v=>v.nonAcademic[1]);direct(18,"Honorarios Charlas Vinculación Empresas",v=>v.nonAcademic[2]);direct(19,"Revisión y seguimiento de contenidos virtuales",()=>0);direct(20,"Creación de contenidos virtuales Competencias Genéricas",()=>0);rows.push(row(21,[tc("A21","HONORARIOS NO ACADÉMICOS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}21`,`SUM(${col(i+2)}16:${col(i+2)}20)`,v.f.nonAcademicHonoraria,S.subtotal))]));rows.push(row(22,[]));direct(23,"Materiales básicos de enseñanza",()=>0);direct(24,"Textos impresos Bibliografía",v=>v.books);rows.push(row(25,[tc("A25","LIBROS Y PUBLICACIONES TÉCNICAS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}25`,`SUM(${col(i+2)}23:${col(i+2)}24)`,v.books,S.subtotal))]));rows.push(row(26,[]));direct(27,"Difusión",v=>v.diff);rows.push(row(28,[tc("A28","DIFUSIÓN",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}28`,`${col(i+2)}27`,v.diff,S.subtotal))]));rows.push(row(29,[]));direct(30,"Docentes internacional",()=>0);direct(31,"Movilidad académicos / Pasajes y fletes",v=>v.travel);rows.push(row(32,[tc("A32","PASAJES Y FLETES",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}32`,`SUM(${col(i+2)}30:${col(i+2)}31)`,v.travel,S.subtotal))]));rows.push(row(33,[]));direct(34,"Licencias de software",v=>v.software);rows.push(row(35,[tc("A35","ADQUISICIÓN DE PROGRAMAS O LICENCIAS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}35`,`${col(i+2)}34`,v.software,S.subtotal))]));rows.push(row(36,[]));direct(37,"Giro para rendir (gastos menores)",v=>v.oper);direct(38,"Seminarios nacionales y/o internacionales (coffee break)",()=>0);direct(39,"Coffee break otros eventos",v=>v.food);rows.push(row(40,[tc("A40","OTROS SERVICIOS",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}40`,`SUM(${col(i+2)}37:${col(i+2)}39)`,v.oper+v.food,S.subtotal))]));rows.push(row(41,[]));direct(42,supportLabel,v=>v.congress);direct(43,"Beca de atención Económica (manutención)",v=>v.aid);rows.push(row(44,[tc("A44","APOYO INSTITUCIONAL A ESTUDIANTES",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}44`,`SUM(${col(i+2)}42:${col(i+2)}43)`,v.congress+v.aid,S.subtotal))]));rows.push(row(45,[]));direct(46,"Inversión computacional",v=>v.equip);direct(47,"Inversión otro equipamiento",()=>0);rows.push(row(48,[tc("A48","INVERSIONES",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}48`,`SUM(${col(i+2)}46:${col(i+2)}47)`,v.equip,S.subtotal))]));rows.push(row(49,[tc("A49","TOTAL COSTOS Y GASTOS DE ADMINISTRACIÓN",S.subtotal),...vals.map((v,i)=>fc(`${col(i+2)}49`,`${col(i+2)}14+${col(i+2)}21+${col(i+2)}25+${col(i+2)}28+${col(i+2)}32+${col(i+2)}35+${col(i+2)}40+${col(i+2)}44+${col(i+2)}48`,v.f.totalExpenses,S.subtotal))]));rows.push(row(50,[tc("A50","FLUJO DE CAJA NETO",S.bold),...vals.map((v,i)=>fc(`${col(i+2)}50`,`${col(i+2)}8-${col(i+2)}49`,v.f.netFlow,S.result))]));return sheet(`A1:${last}50`,rows,`<col min="1" max="1" width="64" customWidth="1"/><col min="2" max="${ys.length+1}" width="17" customWidth="1"/>`,`A1:${last}1`);}

const styles=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="$#,##0;[Red]($#,##0);-"/><numFmt numFmtId="165" formatCode="#0.##;[Red](#0.##);-"/></numFmts><fonts count="3"><font><sz val="10"/><name val="Arial"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Arial"/></font><font><b/><sz val="10"/><name val="Arial"/></font></fonts><fills count="7"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF1F4E78"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF245781"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF0F7C82"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFD9EAF7"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFE2F0D9"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="9"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="3" borderId="0" xfId="0"><alignment horizontal="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="164" fontId="2" fillId="5" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="0" fontId="1" fillId="4" borderId="0" xfId="0"/><xf numFmtId="165" fontId="0" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="164" fontId="2" fillId="6" borderId="0" xfId="0" applyNumberFormat="1"/><xf numFmtId="0" fontId="2" fillId="0" borderId="0" xfId="0"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;

export function createDoctoralBudgetXlsx(budget:CohortBudget,result:BudgetResult,parameters:InstitutionalParameters):Uint8Array{const sheets=[{name:"Parámetros",xml:parametersSheet(budget,result,parameters)},{name:"Flujo de estudiantes",xml:studentsSheet(budget,result,parameters)},{name:"Costo Directo de Docencia",xml:teachingSheet(budget,result,parameters)},{name:"Flujo Total",xml:totalSheet(budget,result)}],overrides=sheets.map((_,i)=>`<Override PartName="/xl/worksheets/sheet${i+1}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>`).join(""),defs=sheets.map((s,i)=>`<sheet name="${esc(s.name)}" sheetId="${i+1}" r:id="rId${i+1}"/>`).join(""),rels=sheets.map((_,i)=>`<Relationship Id="rId${i+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${i+1}.xml"/>`).join(""),now=new Date().toISOString(),files:Array<{name:string;data:string|Uint8Array}>=[{name:"[Content_Types].xml",data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>${overrides}<Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/></Types>`},{name:"_rels/.rels",data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/></Relationships>`},{name:"docProps/core.xml",data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>Presupuesto ${esc(budget.program.name)}</dc:title><dc:creator>UTEM · Escuela de Postgrado</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${now}</dcterms:created></cp:coreProperties>`},{name:"xl/workbook.xml",data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><bookViews><workbookView activeTab="3" firstSheet="0"/></bookViews><sheets>${defs}</sheets><calcPr calcId="191029" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>`},{name:"xl/_rels/workbook.xml.rels",data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${rels}<Relationship Id="rId${sheets.length+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`},{name:"xl/styles.xml",data:styles}];sheets.forEach((s,i)=>files.push({name:`xl/worksheets/sheet${i+1}.xml`,data:s.xml}));return zip(files);}

export const createAcademicBudgetXlsx = createDoctoralBudgetXlsx;
