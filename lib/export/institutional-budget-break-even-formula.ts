...
function setFormula(sheetXml: string, ref: string, formula: string, cached: number): string {
  ...
  return sheetXml.replace(pattern, () => `<c r="${ref}"${style}><f>${xml(formula)}</f><v>${Number.isFinite(cached) ? cached : 0}</v></c>`);
}
...
/**
 * Corrige únicamente la fórmula de punto de equilibrio dentro del XLSX institucional.
 * No agrega hojas, filas, columnas, estilos ni cambia el modelo histórico de Postgrado.
 *
 * La identidad es exactamente la misma del motor v13.0.1:
 * resultado operacional = equivalentes × aporte por equivalente - costos fijos.
 *
 * Se evita LET deliberadamente para que Excel no introduzca el operador de intersección
 * implícita "@" al abrir el archivo en determinadas versiones.
 */
export async function alignInstitutionalBreakEvenFormula(
  ...
  const lastYearColumn = yearColumn(Math.max(0, result.years.length - 1));
  ...
  const contribution = `SUM('FLUJO TOTAL'!${firstYearColumn}5:${lastYearColumn}5)+SUM('FLUJO TOTAL'!${firstYearColumn}6:${lastYearColumn}6)+SUM('Flujo estudiantes'!${firstYearColumn}${enrollmentIncomeRow}:${lastYearColumn}${enrollmentIncomeRow})*${recognition}+SUM('FLUJO TOTAL'!${firstYearColumn}36:${lastYearColumn}36)+SUM('FLUJO TOTAL'!${firstYearColumn}10:${lastYearColumn}10)`;
  const formula = `IFERROR(${fixedCosts}*${firstYearColumn}${equivalentStudentsRow}/(${contribution}),0)`;

  let sheet = decoder.decode(files.get(sheetName)!);
  sheet = setFormula(sheet, `${firstYearColumn}${equilibriumRow}`, formula, equilibrium.minimumEquivalentEnrollmentsExact ?? 0);
