...
export function buildFinancialNarrative(
  ...
  const enrollmentInformative = sum(result.annualFlows.map((flow) => flow.grossEnrollmentFee));
  ...
  const antecedents = `La cohorte analizada corresponde a ${budget.program.name}, versión ${budget.programVersionLabel}, con inicio ${budget.startYear}-${budget.startSemester}S y una duración presupuestada de ${budget.durationSemesters} semestres (${periodText}). La formulación considera ${current.initialStudents} estudiante(s) inicialmente matriculados o proyectados y ${current.activeStudents} estudiante(s) vigentes en el último semestre presupuestado. El valor de matrícula se presenta como antecedente informativo por un total bruto de ${money(enrollmentInformative)} durante el horizonte y no se incorpora al total de ingresos del presupuesto.`;

  const income = `Los ingresos brutos por aranceles alcanzan ${money(current.grossTuition)} para el ciclo completo. Sobre dicho monto se aplican becas y descuentos por ${money(current.discountsAndScholarships)} e incobrabilidad por ${money(current.badDebt)}. Los ingresos externos u otros ajustes positivos ascienden a ${money(externalIncome)}. Como resultado, los ingresos netos presupuestados totalizan ${money(current.netIncome)}, equivalentes a ${moneyOrNA(current.incomePerStudent)} por estudiante inicial. El detalle anual es: ${annualIncomeDetail(budget, result, parameters)}.`;
