...
import { grossEnrollmentPerStudentForProgram, normalizedTuitionDistribution, proportionalTuitionDistribution, rawTuitionDistributionTotal, tuitionInstallmentValue } from "@/lib/calculations/billing";
...
const FUNCTIONAL_RELEASE = "v12.1.1";
...
function freshBudget(program: Program, responsible: string, parameters: InstitutionalParameters): CohortBudget {
  ...
  const base: CohortBudget = {
    ...
    enrollmentBillingMode: "ANNUAL",
    programTotalTuition: 0,
    singleEnrollmentFee: 0,
    semesterEnrollmentFee: 0,
    tuitionInstallments: 1,
    tuitionDistributionMode: "PROPORTIONAL",
    tuitionSemesterDistribution: proportionalTuitionDistribution(duration),
    curriculumSectionOverrides: {},
...
export function BudgetWorkspace() {
  ...
  const programTotalPricing = budget?.tuitionPricingMode === "PROGRAM_TOTAL";
  const tuitionDistribution = budget ? normalizedTuitionDistribution(budget) : [];
  const activeBillingPeriods = budget ? getActivePeriods(budget.startYear, budget.startSemester, budget.durationSemesters) : [];
  const rawTuitionDistributionSum = budget ? rawTuitionDistributionTotal(budget) : 0;
  const programTotalTuition = Math.max(0, budget?.programTotalTuition ?? 0);
  const totalEnrollmentPerStudent = budget ? grossEnrollmentPerStudentForProgram(budget, (year) => budget.annualOverrides.find((annual) => annual.year === year)?.annualEnrollmentFee ?? parameters.annualEnrollmentFee[year] ?? 0) : 0;
  const equivalentTuitionPerSemester = budget?.durationSemesters ? programTotalTuition / budget.durationSemesters : 0;
  const installmentValue = budget ? tuitionInstallmentValue(programTotalTuition, 0, budget.tuitionInstallments ?? 1) : 0;
  const cohortTotalExpected = result?.annualFlows.reduce((sum, flow) => sum + flow.totalIncome, 0) ?? 0;
  ...
  function updateSemester(index: number, field: keyof SemesterParameters, value: number | string) {
    if (!budget) return;
    const next = { ...budget, semesters: budget.semesters.map((semester, candidate) => candidate === index ? { ...semester, [field]: value } : semester) };
    replaceBudget(field === "activeStudents" ? applyProgramCurriculumToBudget(next) : next);
  }
  ...
  function setInitialStudentsForAllSemesters(initialStudents: number) {
    ...
    replaceBudget(applyProgramCurriculumToBudget({
      ...budget,
  ...
  function updateCurriculumSections(courseId: string, sections: number) {
    if (!budget) return;
    const overrides = { ...(budget.curriculumSectionOverrides ?? {}), [courseId]: Math.max(0, Math.round(sections)) };
    replaceBudget(applyProgramCurriculumToBudget({ ...budget, curriculumSectionOverrides: overrides }));
  }
  ...
  function addDiscount() {
    ...
    patchBudget({
      discounts: [...budget.discounts, {
        id: uid("discount"), name: "Nuevo descuento", target: "TUITION", percentage: 0, students: 0,
        ...range,
      }],
    });
  }
    ...
    try {
      const response = await fetch("/api/budgets", {
        ...
        body: JSON.stringify({
          ...
          tuitionPricingMode: draft.tuitionPricingMode, enrollmentBillingMode: draft.enrollmentBillingMode,
          programTotalTuition: draft.programTotalTuition, singleEnrollmentFee: draft.singleEnrollmentFee, semesterEnrollmentFee: draft.semesterEnrollmentFee,
          tuitionInstallments: draft.tuitionInstallments, tuitionDistributionMode: draft.tuitionDistributionMode, tuitionSemesterDistribution: draft.tuitionSemesterDistribution, curriculumSectionOverrides: draft.curriculumSectionOverrides ?? {},
          badDebtRate: draft.badDebtRate,
    ...
    try {
      const response = await fetch(`/api/budgets/${budget.id}`, {
        ...
        body: JSON.stringify({
          ...
          tuitionPricingMode: budget.tuitionPricingMode ?? "ANNUAL_LEGACY",
          enrollmentBillingMode: budget.enrollmentBillingMode ?? "ANNUAL",
          programTotalTuition: budget.programTotalTuition ?? 0,
          singleEnrollmentFee: budget.singleEnrollmentFee ?? 0,
          semesterEnrollmentFee: budget.semesterEnrollmentFee ?? 0,
          tuitionInstallments: budget.tuitionInstallments ?? 1,
          tuitionDistributionMode: budget.tuitionDistributionMode ?? "PROPORTIONAL",
          tuitionSemesterDistribution: budget.tuitionSemesterDistribution ?? proportionalTuitionDistribution(budget.durationSemesters),
          curriculumSectionOverrides: budget.curriculumSectionOverrides ?? {},
  ...
  async function cloneBudget() {
    ...
    try {
      ...
      const created = await responseBody<{ id: string }>(await fetch("/api/budgets", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ programId: budget.program.id, cohortName: clonedCohortName, startYear: budget.startYear, startSemester: budget.startSemester, durationSemesters: budget.durationSemesters, initialStudents: budget.initialStudents, facultyOverheadRate: budget.facultyOverheadRate, enrollmentRecognitionRate: budget.enrollmentRecognitionRate, tuitionPricingMode: budget.tuitionPricingMode ?? "ANNUAL_LEGACY", enrollmentBillingMode: budget.enrollmentBillingMode ?? "ANNUAL", programTotalTuition: budget.programTotalTuition ?? 0, singleEnrollmentFee: budget.singleEnrollmentFee ?? 0, semesterEnrollmentFee: budget.semesterEnrollmentFee ?? 0, tuitionInstallments: budget.tuitionInstallments ?? 1, tuitionDistributionMode: budget.tuitionDistributionMode ?? "PROPORTIONAL", tuitionSemesterDistribution: budget.tuitionSemesterDistribution ?? proportionalTuitionDistribution(budget.durationSemesters), curriculumSectionOverrides: budget.curriculumSectionOverrides ?? {}, badDebtRate: Number.isFinite(budget.badDebtRate) ? budget.badDebtRate : null, programVersionLabel: budget.programVersionLabel, scholarshipsEnabled: budget.scholarshipsEnabled, deliveryModality: budget.deliveryModality, annualOverrides: budget.annualOverrides, authorizedInitialCarryover: budget.authorizedInitialCarryover, includeAuthorizedCarryover: budget.includeAuthorizedCarryover, normalizeSharedCosts: budget.normalizeSharedCosts, alertPotentialDuplicates: budget.alertPotentialDuplicates, appliedTemplateId: budget.appliedTemplateId ?? null, appliedTemplateVersion: budget.appliedTemplateVersion ?? null, notes: `Clonado desde ${budget.cohortName}`, responsibleId: identity.userId }) }));
      await responseBody(await fetch(`/api/budgets/${created.id}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ deliveryModality: budget.deliveryModality, tuitionPricingMode: budget.tuitionPricingMode ?? "ANNUAL_LEGACY", enrollmentBillingMode: budget.enrollmentBillingMode ?? "ANNUAL", programTotalTuition: budget.programTotalTuition ?? 0, singleEnrollmentFee: budget.singleEnrollmentFee ?? 0, semesterEnrollmentFee: budget.semesterEnrollmentFee ?? 0, tuitionInstallments: budget.tuitionInstallments ?? 1, tuitionDistributionMode: budget.tuitionDistributionMode ?? "PROPORTIONAL", tuitionSemesterDistribution: budget.tuitionSemesterDistribution ?? proportionalTuitionDistribution(budget.durationSemesters), curriculumSectionOverrides: budget.curriculumSectionOverrides ?? {}, annualOverrides: budget.annualOverrides, semesters: budget.semesters.map((semester, position) => ({ ...semester, position })), discounts: budget.discounts, externalIncome: budget.externalIncome, items: budget.manualItems, sharedCourses: budget.sharedCourses, notes: `Clonado desde ${budget.program.code} · ${budget.cohortName}`, changeNote: "Clonación de presupuesto" }) }));
  ...
  function setTuitionPricingMode(mode: "ANNUAL_LEGACY" | "PROGRAM_TOTAL") {
    if (!budget) return;
    if (mode === "ANNUAL_LEGACY") {
      patchBudget({ tuitionPricingMode: mode });
      return;
    }
    const legacyTotal = getAnnualEnrollmentChargePeriods(budget.startYear, budget.startSemester, budget.durationSemesters)
      .reduce((sum, period) => sum + (budget.annualOverrides.find((annual) => annual.year === period.year)?.annualTuition ?? 0), 0);
    patchBudget({
      tuitionPricingMode: mode,
      programTotalTuition: Math.max(0, (budget.programTotalTuition ?? 0) > 0 ? (budget.programTotalTuition ?? 0) : legacyTotal),
      tuitionInstallments: Math.max(1, budget.tuitionInstallments ?? budget.durationSemesters * 6),
      tuitionDistributionMode: budget.tuitionDistributionMode ?? "PROPORTIONAL",
      tuitionSemesterDistribution: budget.tuitionSemesterDistribution?.length === budget.durationSemesters
        ? budget.tuitionSemesterDistribution
        : proportionalTuitionDistribution(budget.durationSemesters),
    });
  }

  function setEnrollmentBillingMode(mode: "ANNUAL" | "SINGLE_SPECIAL" | "SEMESTER") {
    if (!budget) return;
    const firstAnnualFee = budget.annualOverrides[0]?.annualEnrollmentFee ?? 0;
    patchBudget({
      enrollmentBillingMode: mode,
      singleEnrollmentFee: budget.singleEnrollmentFee ?? firstAnnualFee,
      semesterEnrollmentFee: budget.semesterEnrollmentFee ?? 0,
    });
  }

  function setTuitionDistributionMode(mode: "PROPORTIONAL" | "CUSTOM") {
    if (!budget) return;
    patchBudget({
      tuitionDistributionMode: mode,
      tuitionSemesterDistribution: mode === "PROPORTIONAL"
        ? proportionalTuitionDistribution(budget.durationSemesters)
        : (budget.tuitionSemesterDistribution?.length === budget.durationSemesters
          ? budget.tuitionSemesterDistribution
          : proportionalTuitionDistribution(budget.durationSemesters)),
    });
  }

  function updateTuitionSemesterShare(index: number, percentage: number) {
    if (!budget) return;
    const current = Array.from({ length: budget.durationSemesters }, (_, candidate) => budget.tuitionSemesterDistribution?.[candidate] ?? 0);
    current[index] = Math.max(0, percentage / 100);
    patchBudget({ tuitionDistributionMode: "CUSTOM", tuitionSemesterDistribution: current });
  }
  ...
  return <div className="budget-workspace">
    ...
    <section className="panel">
      ...
      <div className="subpanel annual-parameter-panel billing-architecture-panel">
        <h3>Estructura de cobro</h3>
        <p>El precio académico y su reconocimiento presupuestario se administran por separado. El año calendario determina cuándo se reconoce el ingreso, no el precio total del programa.</p>
        <div className="engine-architecture-strip" aria-label="Jerarquía del motor financiero"><span><b>1</b> Precio del programa</span><i aria-hidden="true">→</i><span><b>2</b> Distribución semestral</span><i aria-hidden="true">→</i><span><b>3</b> Reconocimiento anual</span><i aria-hidden="true">→</i><span><b>4</b> Flujo financiero</span></div>
        <div className="form-grid cols-4">
          <label>Modelo de arancel<select disabled={!editable} value={budget.tuitionPricingMode ?? "ANNUAL_LEGACY"} onChange={(event) => setTuitionPricingMode(event.target.value as "ANNUAL_LEGACY" | "PROGRAM_TOTAL")}><option value="ANNUAL_LEGACY">Arancel anual (compatibilidad)</option><option value="PROGRAM_TOTAL">Arancel total del programa</option></select><small>Los presupuestos históricos permanecen en modalidad anual.</small></label>
          <label>Modalidad de matrícula<select disabled={!editable} value={budget.enrollmentBillingMode ?? "ANNUAL"} onChange={(event) => setEnrollmentBillingMode(event.target.value as "ANNUAL" | "SINGLE_SPECIAL" | "SEMESTER")}><option value="ANNUAL">Matrícula anual</option><option value="SINGLE_SPECIAL">Matrícula única / especial</option><option value="SEMESTER">Matrícula semestral</option></select></label>
          {programTotalPricing ? <><label>Arancel total del programa<input disabled={!editable} type="number" min="0" value={budget.programTotalTuition ?? 0} onChange={(event) => patchBudget({ programTotalTuition: numberValue(event.target.value) })} /><small>Precio bruto completo de los {budget.durationSemesters} semestres.</small></label><label>Número de cuotas del arancel<input disabled={!editable} type="number" min="1" max="120" value={budget.tuitionInstallments ?? 1} onChange={(event) => patchBudget({ tuitionInstallments: Math.max(1, Math.round(numberValue(event.target.value))) })} /><small>La cantidad de cuotas no modifica el ingreso total.</small></label></> : null}
          {budget.enrollmentBillingMode === "SINGLE_SPECIAL" ? <label>Valor matrícula única / especial<input disabled={!editable} type="number" min="0" value={budget.singleEnrollmentFee ?? 0} onChange={(event) => patchBudget({ singleEnrollmentFee: numberValue(event.target.value) })} /><small>Se cobra sólo en el primer semestre activo.</small></label> : null}
          {budget.enrollmentBillingMode === "SEMESTER" ? <label>Valor matrícula por semestre<input disabled={!editable} type="number" min="0" value={budget.semesterEnrollmentFee ?? 0} onChange={(event) => patchBudget({ semesterEnrollmentFee: numberValue(event.target.value) })} /><small>Total por estudiante: {formatCLP((budget.semesterEnrollmentFee ?? 0) * budget.durationSemesters)}.</small></label> : null}
        </div>
        {programTotalPricing ? <>
          <div className="summary-grid">
            <div><span>Duración</span><strong>{budget.durationSemesters} semestres</strong></div>
            <div><span>Arancel total</span><strong>{formatCLP(programTotalTuition)}</strong></div>
            <div><span>Arancel equivalente / semestre</span><strong>{formatCLP(equivalentTuitionPerSemester)}</strong></div>
            <div><span>Cuota base aproximada</span><strong>{formatCLP(installmentValue)}</strong><small>Antes de descuentos particulares.</small></div>
            <div><span>Matrícula total / estudiante</span><strong>{formatCLP(totalEnrollmentPerStudent)}</strong></div>
            <div><span>Costo bruto total / estudiante</span><strong>{formatCLP(programTotalTuition + totalEnrollmentPerStudent)}</strong></div>
          </div>
          <div className="form-grid cols-4"><label>Distribución del arancel<select disabled={!editable} value={budget.tuitionDistributionMode ?? "PROPORTIONAL"} onChange={(event) => setTuitionDistributionMode(event.target.value as "PROPORTIONAL" | "CUSTOM")}><option value="PROPORTIONAL">Proporcional por semestre</option><option value="CUSTOM">Personalizada</option></select></label><div className="field-action"><span>Total distribución</span><strong>{(rawTuitionDistributionSum * 100).toLocaleString("es-CL", { maximumFractionDigits: 2 })}%</strong><small>{budget.tuitionDistributionMode === "CUSTOM" && Math.abs(rawTuitionDistributionSum - 1) > 0.0001 ? "Debe sumar exactamente 100%." : "Distribución válida."}</small></div></div>
          <div className="table-wrap"><table className="data-table editable-list"><thead><tr><th>Semestre del programa</th><th>Periodo presupuestario</th><th>Distribución</th><th>Arancel bruto por estudiante</th></tr></thead><tbody>{activeBillingPeriods.map((period, index) => { const rawShare = budget.tuitionDistributionMode === "CUSTOM" ? (budget.tuitionSemesterDistribution?.[index] ?? 0) : (1 / budget.durationSemesters); return <tr key={`tuition-share-${period.year}-${period.semester}`}><th>Semestre {index + 1}</th><td>{period.year}-{period.semester}S</td><td>{budget.tuitionDistributionMode === "CUSTOM" ? <div className="percent-input"><input disabled={!editable} aria-label={`Distribución arancel semestre ${index + 1}`} type="number" min="0" max="100" step="0.1" value={(rawShare * 100).toFixed(1)} onChange={(event) => updateTuitionSemesterShare(index, numberValue(event.target.value))} /><span>%</span></div> : formatPercent(rawShare)}</td><td className="numeric">{formatCLP(programTotalTuition * rawShare)}</td></tr>; })}</tbody></table></div>
          <div className="notice info"><strong>Proyección de ingresos de la cohorte</strong><p>Arancel bruto: {formatCLP(result.annualFlows.reduce((sum, flow) => sum + flow.grossTuition, 0))} · Descuentos de arancel: {formatCLP(result.annualFlows.reduce((sum, flow) => sum + flow.discounts, 0))} · Arancel neto esperado: {formatCLP(result.annualFlows.reduce((sum, flow) => sum + flow.netTuitionIncome, 0))} · Matrícula bruta: {formatCLP(result.annualFlows.reduce((sum, flow) => sum + flow.grossEnrollmentFee, 0))} · Matrícula reconocida como ingreso: {formatCLP(result.annualFlows.reduce((sum, flow) => sum + flow.recognizedEnrollmentFee, 0))} · Ingreso total presupuestario: {formatCLP(cohortTotalExpected)}.</p></div>
          {budget.discounts.some((discount) => (discount.target ?? "TUITION") === "TUITION" && discount.percentage > 0) ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Beneficio sobre arancel</th><th>Descuento</th><th>Arancel efectivo / estudiante</th><th>Cuota aproximada</th></tr></thead><tbody>{budget.discounts.filter((discount) => (discount.target ?? "TUITION") === "TUITION" && discount.percentage > 0).map((discount) => <tr key={`pricing-discount-${discount.id}`}><th>{discount.name}</th><td>{formatPercent(discount.percentage)}</td><td className="numeric">{formatCLP(programTotalTuition * (1 - discount.percentage))}</td><td className="numeric">{formatCLP(tuitionInstallmentValue(programTotalTuition, discount.percentage, budget.tuitionInstallments ?? 1))}</td></tr>)}</tbody></table></div> : null}
          <div className="table-wrap"><table className="data-table"><thead><tr><th>Comparador</th><th>Matrícula total / estudiante</th><th>Arancel total</th><th>Costo total estudiante</th><th>Costo bruto cohorte</th><th>Distribución por año presupuestario</th></tr></thead><tbody><tr><th>Alternativa A · Matrícula única / especial</th><td className="numeric">{formatCLP(budget.singleEnrollmentFee ?? 0)}</td><td className="numeric">{formatCLP(programTotalTuition)}</td><td className="numeric">{formatCLP(programTotalTuition + (budget.singleEnrollmentFee ?? 0))}</td><td className="numeric">{formatCLP((programTotalTuition + (budget.singleEnrollmentFee ?? 0)) * budget.initialStudents)}</td><td>{result.years.map((year) => { const tuitionShare = tuitionDistribution.filter((period) => period.year === year).reduce((sum, period) => sum + period.share, 0); const enrollment = year === budget.startYear ? (budget.singleEnrollmentFee ?? 0) : 0; return `${year}: ${formatCLP((programTotalTuition * tuitionShare + enrollment) * budget.initialStudents)}`; }).join(" · ")}</td></tr><tr><th>Alternativa B · Matrícula semestral</th><td className="numeric">{formatCLP((budget.semesterEnrollmentFee ?? 0) * budget.durationSemesters)}</td><td className="numeric">{formatCLP(programTotalTuition)}</td><td className="numeric">{formatCLP(programTotalTuition + (budget.semesterEnrollmentFee ?? 0) * budget.durationSemesters)}</td><td className="numeric">{formatCLP((programTotalTuition + (budget.semesterEnrollmentFee ?? 0) * budget.durationSemesters) * budget.initialStudents)}</td><td>{result.years.map((year) => { const tuitionShare = tuitionDistribution.filter((period) => period.year === year).reduce((sum, period) => sum + period.share, 0); const semesterCount = activeBillingPeriods.filter((period) => period.year === year).length; return `${year}: ${formatCLP((programTotalTuition * tuitionShare + (budget.semesterEnrollmentFee ?? 0) * semesterCount) * budget.initialStudents)}`; }).join(" · ")}</td></tr></tbody></table></div>
        </> : null}
      </div>

      {!programTotalPricing ? <div className="subpanel annual-parameter-panel"><h3>Valores anuales del presupuesto</h3><p>Modo de compatibilidad para presupuestos históricos. El arancel y la matrícula mantienen su lógica anual existente.</p>
      ...
      </div> : <div className="subpanel annual-parameter-panel"><h3>Parámetros anuales complementarios</h3><p>El arancel ya está definido como precio total del programa. Aquí se mantienen únicamente los parámetros anuales que dependen del año presupuestario.</p><div className="table-wrap"><table className="data-table editable-list"><thead><tr><th>Año</th>{budget.enrollmentBillingMode === "ANNUAL" ? <th>Matrícula anual</th> : null}<th>Guía de tesis por graduando</th></tr></thead><tbody>{budget.annualOverrides.map((annual) => <tr key={`annual-support-${annual.year}`}><th>{annual.year}</th>{budget.enrollmentBillingMode === "ANNUAL" ? <InputCell label={`Matrícula anual ${annual.year}`} value={annual.annualEnrollmentFee} disabled={!editable} onChange={(value) => updateAnnualOverride(annual.year, { annualEnrollmentFee: value })} /> : null}<InputCell label={`Guía de tesis ${annual.year}`} value={annual.thesisGuidancePerGraduatingStudent} disabled={!editable} onChange={(value) => updateAnnualOverride(annual.year, { thesisGuidancePerGraduatingStudent: value })} /></tr>)}</tbody></table></div></div>}
    ...
    <section className="panel"><SectionHeading number="4" id="carga-academica" title="Carga académica" description={budget.program.curriculumCourses?.length ? `La malla del programa contiene ${budget.program.curriculumCourses.length} registros. Sincronícela para recalcular horas y economías de escala.` : "La modalidad define qué horas docentes se valorizan; las horas de reemplazo se mantienen separadas."} action={budget.program.curriculumCourses?.length ? <div className="section-action-group"><button className="button secondary" type="button" disabled={!editable || curriculumApplying} onClick={() => void applyCurriculumToActiveBudget(false)}>{curriculumApplying ? "Aplicando…" : "Aplicar malla curricular"}</button>{budget.program.type === "MAGISTER_PROFESIONAL" ? <button className="button secondary" type="button" disabled={!editable || curriculumApplying} onClick={() => void applyCurriculumToActiveBudget(true)}>Sugerir equilibrio</button> : null}</div> : undefined} />
      {budget.program.curriculumCourses?.length ? <div className="notice info"><strong>Malla curricular vinculada</strong><p>Sólo las horas de docencia directa de la malla alimentan el presupuesto; las horas autónomas no se incorporan al flujo. Electivos, especialización y asignaturas de graduación (Tesis/AFE) permiten modificar secciones por cohorte. En Doctorados y Magísteres Académicos, las asignaturas de graduación toman por defecto una sección por estudiante activo del semestre, salvo ajuste manual.</p></div> : null}
    ...
    <section className="panel">
      <SectionHeading number="6" id="descuentos" title="Descuentos" description="Los descuentos se aplican al arancel por defecto. Use Matrícula sólo cuando exista expresamente un beneficio sobre ese concepto." action={<button className="button secondary" type="button" disabled={!editable} onClick={addDiscount}>Agregar descuento</button>} />
      <div className="table-wrap"><table className="data-table editable-list"><thead><tr><th>Nombre</th><th>Aplicar sobre</th><th>Porcentaje</th><th>Estudiantes</th><th>Inicio</th><th>Término</th><th>Acción</th></tr></thead><tbody>{budget.discounts.length ? budget.discounts.map((discount, index) => <tr key={discount.id}>
        <td><input disabled={!editable} value={discount.name} onChange={(event) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, name: event.target.value } : item) })} /></td>
        <td><select disabled={!editable} value={discount.target ?? "TUITION"} onChange={(event) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, target: event.target.value as "TUITION" | "ENROLLMENT" } : item) })}><option value="TUITION">Arancel</option><option value="ENROLLMENT">Matrícula</option></select></td>
        <PercentCell label={`Descuento ${discount.name}`} value={discount.percentage} disabled={!editable} onChange={(value) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, percentage: value } : item) })} />
        <td><input disabled={!editable} type="number" min="0" value={discount.students} onChange={(event) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, students: numberValue(event.target.value) } : item) })} /></td>
        <td><PeriodInputs disabled={!editable} years={result.years} activePeriods={result.periods} year={discount.startYear} semester={discount.startSemester} onYear={(value) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, startYear: value } : item) })} onSemester={(value) => patchBudget({ discounts: budget.discounts.map((item, candidate) => candidate === index ? { ...item, startSemester: value } : item) })} /></td>
        ...
        <td><button className="text-button danger-text" type="button" disabled={!editable} onClick={() => patchBudget({ discounts: budget.discounts.filter((_, candidate) => candidate !== index) })}>Quitar</button></td>
      </tr>) : <tr><td colSpan={7}>No hay descuentos.</td></tr>}</tbody></table></div>
    </section>
    ...
    <section className="panel">
      ...
      <div className="table-wrap financial-flow">
        <table className="data-table financial-table cashflow-editable-table">
          ...
          <tbody>
            <FlowRow label={budget.enrollmentBillingMode === "SINGLE_SPECIAL" ? "Matrícula única / especial bruta" : budget.enrollmentBillingMode === "SEMESTER" ? "Matrícula semestral bruta" : "Matrícula anual bruta"} values={result.annualFlows.map((flow) => flow.grossEnrollmentFee)} tone="income" />
            {result.annualFlows.some((flow) => flow.enrollmentDiscounts > 0) ? <FlowRow label="Descuentos matrícula" values={result.annualFlows.map((flow) => -flow.enrollmentDiscounts)} tone="income" /> : null}
...
function PeriodInputs({ year, semester, years, activePeriods, onYear, onSemester, disabled }: { year: number; semester: 1 | 2; years: number[]; activePeriods?: Array<{ year: number; semester: 1 | 2 }>; onYear: (value: number) => void; onSemester: (value: 1 | 2) => void; disabled: boolean }) {
  const semestersForYear = activePeriods?.filter((period) => period.year === year).map((period) => period.semester) ?? [1, 2] as Array<1 | 2>;
  return <div className="period-inputs"><select disabled={disabled} value={year} onChange={(event) => { const nextYear = numberValue(event.target.value); onYear(nextYear); const allowed = activePeriods?.filter((period) => period.year === nextYear).map((period) => period.semester) ?? [1, 2] as Array<1 | 2>; if (!allowed.includes(semester) && allowed[0]) onSemester(allowed[0]); }}>{years.map((candidate) => <option key={candidate}>{candidate}</option>)}</select><select disabled={disabled} value={semester} onChange={(event) => onSemester(numberValue(event.target.value) as 1 | 2)}>{semestersForYear.map((candidate) => <option key={candidate} value={candidate}>{candidate}S</option>)}</select></div>;
}
