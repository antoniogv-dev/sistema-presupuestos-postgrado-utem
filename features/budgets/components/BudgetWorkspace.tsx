...
import { applyProgramCurriculumToBudget, curriculumCourseEffectiveHours, curriculumCourseRawHours, curriculumCourseWeeklyDirectHours, payableCurriculumCourses } from "@/lib/curriculum/budget-load";
...
const FUNCTIONAL_RELEASE = "v10.28";
...
export function BudgetWorkspace() {
  ...
  return <div className="budget-workspace">
    ...
    <section className="panel"><SectionHeading number="4" id="carga-academica" title="Carga académica" description={budget.program.curriculumCourses?.length ? `La malla del programa contiene ${budget.program.curriculumCourses.length} registros. Sincronícela para recalcular horas y economías de escala.` : "La modalidad define qué horas docentes se valorizan; las horas de reemplazo se mantienen separadas."} action={budget.program.curriculumCourses?.length ? <div className="section-action-group"><button className="button secondary" type="button" disabled={!editable || curriculumApplying} onClick={() => void applyCurriculumToActiveBudget(false)}>{curriculumApplying ? "Aplicando…" : "Aplicar malla curricular"}</button>{budget.program.type === "MAGISTER_PROFESIONAL" ? <button className="button secondary" type="button" disabled={!editable || curriculumApplying} onClick={() => void applyCurriculumToActiveBudget(true)}>Sugerir equilibrio</button> : null}</div> : undefined} />
      ...
      {curriculumCourses.length ? <div className="subpanel curriculum-applied-panel"><h3>Asignaturas vinculadas a esta formulación</h3><p>Esta tabla permite comprobar qué asignaturas alimentan la carga. “Horas aplicadas” incorpora semanas, secciones y, cuando corresponde, el factor asincrónico.</p><div className="table-wrap"><table className="data-table curriculum-applied-table"><thead><tr><th>Periodo</th><th>Código</th><th>Asignatura</th><th>Tipo</th><th>Modalidad</th><th>Semanas</th><th>Secciones</th><th>Horas/sem.</th><th>Horas aplicadas</th><th>Condición</th></tr></thead><tbody>{curriculumCourses.map((course) => { const semester = budget.semesters[course.semester - 1]; const generic = course.kind === "COMPETENCIA_GENERICA"; const rawHours = generic ? 0 : curriculumCourseRawHours(course); const effectiveHours = generic ? 0 : curriculumCourseEffectiveHours(course, budget.deliveryModality); const condition = generic ? "Competencia genérica · sin costo" : course.sharedWithProgramIds.length ? `Compartida · ${(course.allocationRate * 100).toLocaleString("es-CL", { maximumFractionDigits: 1 })}% imputado` : "Programa"; return <tr key={course.id}><td>{semester ? `${semester.year}-${semester.semester}S` : `Sem. ${course.semester}`}</td><td>{course.code || "—"}</td><td><strong>{course.name}</strong>{course.teachingMode === "ASINCRONICA" ? <small className="cell-note"> Factor {(course.asynchronousRateFactor * 100).toLocaleString("es-CL", { maximumFractionDigits: 1 })}%</small> : null}</td><td>{course.kind.replaceAll("_", " ")}</td><td>{course.teachingMode === "SINCRONICA" ? "Sincrónica" : course.teachingMode === "ASINCRONICA" ? "Asincrónica" : "Presencial"}</td><td className="numeric">{course.weeks}</td><td className="numeric">{course.sections}</td><td className="numeric">{curriculumCourseWeeklyDirectHours(course).toLocaleString("es-CL", { maximumFractionDigits: 2 })}</td><td className="numeric"><strong>{effectiveHours.toLocaleString("es-CL", { maximumFractionDigits: 2 })}</strong>{rawHours !== effectiveHours && rawHours > 0 ? <small className="cell-note"> de {rawHours.toLocaleString("es-CL", { maximumFractionDigits: 2 })} h brutas</small> : null}</td><td>{condition}</td></tr>; })}</tbody></table></div></div> : null}
