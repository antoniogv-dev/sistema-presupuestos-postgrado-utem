...
for (const marker of ["Clonar presupuesto", "Enviar por correo", "Economías de escala", "/api/workflow/recipients", "v10.28"]) {
...
// v10.24: selector de programas vivo y compactación institucional.
for (const marker of [
  'programs.map((program) => <option',
  'setProgramFilterId(nextProgramId)',
  'setDraftBudget(null)',
  'Sin presupuestos para este programa',
  'Actualizar listas',
  'Todos los programas activos creados en el módulo Programas quedan disponibles aquí',
]) {
  if (!budgetWorkspaceV1021.includes(marker)) fail(`Selector de programas v10.24: falta ${marker}.`);
}
for (const marker of ['--black:#000000', '--mustard:#FFB344', '--beige:#FFF8E5', 'compact-selector']) {
  if (!globalsV1021.includes(marker)) fail(`UI institucional v10.24: falta ${marker}.`);
}


// v10.25: exportación profesional con la estructura y fórmulas de la planilla institucional.
const institutionalXlsxV1025 = await readFile(path.join(root, "lib/export/institutional-budget-xlsx.ts"), "utf8");
for (const marker of [
...
for (const marker of [
  "/templates/presupuesto-profesional-formula-base-v10-30.xlsx",
  "cache: \"no-store\"",
  "INSTITUTIONAL_TEMPLATE_SHA256",
  "createInstitutionalFormulaBudgetXlsx",
  "institutionalBudgetFilename",
  "institutionalTemplateCompatibilityIssue",
]) {
  if (!downloadV112.includes(marker)) fail(`download.ts v10.25: falta integración institucional ${marker}.`);
}
const formulaTemplateV1025 = await readFile(path.join(root, "public/templates/presupuesto-profesional-formula-base-v10-30.xlsx"));
...
if (!(packageJson.scripts?.["test:institutional-xlsx"] ?? "").includes("institutional-xlsx.test.mjs")) {
  fail("package.json v10.25: falta test:institutional-xlsx.");
}
...
// v10.30: no reutilizar plantilla XLSX antigua en caché ni volver silenciosamente al exportador general.
for (const marker of [
  "presupuesto-profesional-formula-base-v10-30.xlsx",
  'cache: "no-store"',
  "INSTITUTIONAL_TEMPLATE_SHA256",
...
// v10.28: sincronización curricular visible, sugerencia de equilibrio y defaults de graduación/descuentos.
...
for (const marker of [
  ...
  if (!budgetWorkspaceV1027.includes(marker)) fail(`Presupuestos v10.28: falta ${marker}.`);
}
...
for (const marker of ["graduatingStudents: index === lastIndex ? value", "endYear: last.year", "endSemester: last.semester"]) {
  if (!formDefaultsV1027.includes(marker)) fail(`Defaults v10.28: falta ${marker}.`);
}


// v10.28: importación curricular multinivel y trazabilidad de asignaturas aplicadas.
const curriculumImportV1028 = await readFile(path.join(root, "lib/import/curriculum-file-import.ts"), "utf8");
for (const marker of ["combineHeaderRows", "recognizedHeaderKeys", "best.row + best.depth", "componentDirect"]) {
  if (!curriculumImportV1028.includes(marker)) fail(`Importador curricular v10.28: falta ${marker}.`);
}
const curriculumLoadV1028 = await readFile(path.join(root, "lib/curriculum/budget-load.ts"), "utf8");
for (const marker of ["curriculumCourseWeeklyDirectHours", "theoryWeeklyHours", "laboratoryWeeklyHours", "workshopWeeklyHours"]) {
...
// v10.30: consolidación semestral de malla en la bolsa docente efectiva.
...
for (const marker of ["export function breakEvenExcelFormula", "yearColumnsOrLastColumn: string[] | string", "const currentNetContribution", "const finalBalance", "MAX(0,", "SUM('FLUJO TOTAL'!${firstYearColumn}7:${lastYearColumn}7)", "nonOperationalIncomeTotal", "${firstYearColumn}${resolvedEquivalentStudentsRow}", "matrículas equivalentes", "ROUNDUP(B${equilibriumRow},0)"]) if (!institutionalXlsxV1108.includes(marker)) fail(`XLSX v13.0.10 punto de equilibrio: falta ${marker}.`);
for (const marker of ["v12.1.2 exporta el punto de equilibrio incorporando matrícula y guía de tesis", "IFERROR(MAX(0,B7-('FLUJO TOTAL'!C40*B7/", "SUM('FLUJO TOTAL'!B7:C7)", "ROUNDUP(B14,0)"]) if (!institutionalXlsxTestV1108.includes(marker)) fail(`XLSX v13.0.10 pruebas: falta ${marker}.`);
for (const marker of ["calculateBreakEvenComponents", "flow.totalExpenses - flow.centralOverhead - flow.facultyOverhead - flow.thesisGuidanceCost", "enrollmentPerActualStudent - thesisGuidancePerActualStudent", "actualStudentsReference / equivalentEnrollmentsReference", "contributionPerEquivalentEnrollment", "financialFixedRequirement", "current.finalAccumulatedFlow"]) if (!breakEvenV1108.includes(marker)) fail(`Motor v13.0.10 punto de equilibrio: falta ${marker}.`);
...
// v11.0.11: etiquetas de descuento vinculadas al porcentaje de Parámetros.
const institutionalXlsxV1111 = await readFile(path.join(root, "lib/export/institutional-budget-xlsx.ts"), "utf8");
