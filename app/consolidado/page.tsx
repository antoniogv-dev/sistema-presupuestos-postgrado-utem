...
export default function ConsolidatedPage() {
  const groups = useMemo(() => buildConsolidationGroups(demoBudgets, institutionalParameters), []);
  ...
  const selectedBudgets = group.kind === "PROGRAM" ? demoBudgets.filter((budget) => `program-${budget.program.id}` === group.id) : group.kind === "ACADEMIC" ? demoBudgets.filter((budget) => budget.program.type === "DOCTORADO" || budget.program.type === "MAGISTER_ACADEMICO") : group.kind === "PROFESSIONAL" ? demoBudgets.filter((budget) => budget.program.type === "MAGISTER_PROFESIONAL") : demoBudgets;
  const duplicateAlerts = detectPotentialDuplicateCosts(selectedBudgets);

  return <AppShell><PageHeader eyebrow="Consolidación" title={group.label} description="Consolidado por programa, programas académicos y programas profesionales, con normalización de costos compartidos." actions={<button className="button secondary">Exportar consolidado</button>} />
